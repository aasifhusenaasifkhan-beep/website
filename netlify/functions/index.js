...
    if (!req.file) return res.status(400).json({ error: "Image file mandatory" });

    const fileName = `${uuidv4()}-${req.file.originalname}`;
    const { error: uploadError } = await supabase.storage
      .from("Post-images")
      .upload(fileName, req.file.buffer, { contentType: req.file.mimetype, duplex: "half" });
...
      name, image_url: imageUrl, release_date, genres, season, short_story, category
    }).select();
...
  const epLabel = isNaN(episode_label) ? episode_label : "EPISODE " + String(episode_label).padStart(2, '0');
  const { data, error } = await supabase.from("episodes").insert({ post_id, episode_label: epLabel, original_link }).select();
  if (error) return res.status(500).json({ error: error.message });
...
  if (count >= 3) {
    return res.status(400).json({ error: "Max 3 shorteners are allowed" });
  }
...
    // On-the-fly clean sanitize to prevent user input space errors
