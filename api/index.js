...
app.post("/api/admin/delete-post", adminAuth, async (req, res) => {
  ...
  // Pehle post ki image delete karne ki koshish karein (optional but clean)
  ...
  // Delete post (Foreign Key constraints with ON DELETE CASCADE will auto-delete episodes)
...
app.post("/api/admin/add-premium", adminAuth, async (req, res) => {
  ...
  const expires_at = new Date(Date.now() + 28 * 24 * 60 * 60 * 1000).toISOString(); // 28 Days Expiry
...
app.get("/api/shorten", async (req, res) => {
  ...
  try {
    ...
    // Get configured shorteners
    const { data: shorteners } = await supabase.from("shorteners").select("*");
    if (!shorteners || shorteners.length === 0) {
      // Agar koi shortener nahi hai to direct original link bhej do
      return res.json({ shortLink: originalLink });
    }

    // Serverless counter rotation: Choose a random shortener from DB for equal distribution
